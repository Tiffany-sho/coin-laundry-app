/**
 * プランの**可否と上限**の正。ここが Server Action / API Route の判定に使われる。
 *
 * ⚠️ **画面に出す文言は `planFeatures.js`。** 料金表・設定画面・構造化データ・規約は
 *    すべてあちらから引いている。プランを足すときは
 *    このファイル → `applePlans.js` → `planFeatures.js` の順に触ること。
 */
