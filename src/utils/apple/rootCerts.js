/**
 * Apple のルート CA（公開証明書）。https://www.apple.com/certificateauthority/ から取得したもの。
 *
 * StoreKit の署名済みトランザクション（JWS）は x5c に「リーフ → 中間 → ルート」の
 * 鎖を載せてくる。SignedDataVerifier はその鎖の根がここに載っている証明書と
 * 一致するかを見る。**ここが空だと検証が常に失敗する。**
 *
 * ⚠️ .cer を fs.readFileSync で読まないこと。Vercel のサーバーレス関数は
 *    import されていないファイルをバンドルに含めないので、ローカルでは動いて
 *    本番だけ ENOENT になる。base64 で JS に埋めておけば必ず付いてくる。
 *
 * 中身は公開情報なので、リポジトリに置いて問題ない。
 */
...
/** SignedDataVerifier に渡す形（DER の Buffer 配列） */
export const APPLE_ROOT_CAS = [APPLE_ROOT_CA_G2_BASE64, APPLE_ROOT_CA_G3_BASE64].map((b64) =>
