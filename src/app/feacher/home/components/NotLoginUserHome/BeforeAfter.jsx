...
const beforeItems = [
  { icon: "📝", text: "集金金額を紙のノートやExcelに手書き・手入力" },
  { icon: "📞", text: "スタッフから電話・LINEで口頭報告を受け取る" },
  { icon: "📦", text: "洗剤・柔軟剤の在庫切れに現地へ行って気づく" },
  { icon: "🔧", text: "どの機器が故障中かすぐに答えられない" },
  { icon: "📊", text: "月次集計に毎月1〜2時間かかってしまう" },
];

const afterItems = [
  { icon: "📱", text: "スマホからその場でタップして即記録・即集計" },
  { icon: "📊", text: "リアルタイムで売上グラフが自動生成される" },
  { icon: "🔔", text: "在庫が閾値を下回ったら自動でアラート表示" },
  { icon: "✅", text: "全店舗の機器状態を一覧でいつでも確認できる" },
  { icon: "⚡", text: "月次レポートはボタン1つでCSV/Excelに出力" },
];
...
const ItemRow = ({ icon, text, isBefore }) => (
  ...
  >
    <Text fontSize="lg" flexShrink={0} mt="1px">{icon}</Text>
