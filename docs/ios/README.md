...
| 2026-08-03 | **経費も担当店舗（011）で絞り、権限を 3 段に分ける。** 担当外の店舗の経費は一覧に出さない。⚠️ **組織全体（`laundry_id` が NULL）は誰にも残す**（店舗の経費ではないため。落とすと税理士費用のような支出が admin 以外に見えなくなる）。⚠️ **強制は `getStores()` ではなく `expenses/action.js` の中**（店舗 id の集合では「組織全体」を表せない）。⚠️ **単発の登録は集金担当者にも許し、編集・削除と固定費の管理は admin だけ**にする。現場で出た支出をその場で記録できることを優先し、訂正は admin に寄せた引き換え | [8章](08-data-model.md) |
